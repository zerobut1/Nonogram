...
**提示区布局规则**:
- 提示区不再使用固定正方形留白，而是根据当前题目的 `rowHints` 与 `colHints` 动态计算左侧宽度和顶部高度。
- 左侧宽度基于行提示文本总宽度、数字间距和水平内边距计算，避免多段提示被裁切。
- 顶部高度基于列提示字体高度、垂直间距和上下内边距计算，避免提示过高或过松散。
- 行提示与列提示使用统一字号规则，保持视觉一致；顶部与左侧只在排布方向和间距计算上区分。
- 网格起点、提示背景区域、鼠标坐标换算必须共享同一套布局参数，避免画布尺寸变化后出现显示与点击区域错位。
...
游戏主界面包含：
- **左侧导航栏**: 品牌区、主导航、谜题入口按钮、辅助链接
- **中间主区域**:
  - 顶部标题与游戏模式切换（自由模式 / 纠错模式）
  - 棋盘信息条（当前操作、谜题类型）
  - Canvas 游戏区域（网格 + 提示数字）
  - 底部状态栏（时间、完成度、快捷键）
- **右侧操作区**:
...
| 纠错模式 | 错误输入会先停顿 0.2 秒，再自动纠正为正确状态，并永久显示为红色；再次点击已经处于当前模式目标状态的格子时不会取消 |

### 9.3 自动补叉规则

- 当某一行的所有黑格都已经与答案完全一致时，该行剩余仍为 `EMPTY` 的非黑格会自动补成 `X`。
- 当某一列的所有黑格都已经与答案完全一致时，该列剩余仍为 `EMPTY` 的非黑格会自动补成 `X`。
- 自动补叉只作用于当前空白格，不会覆盖玩家已经填入的黑格或 `X` 标记。
- 同一次玩家操作触发的自动补叉会合并到同一条历史记录中，撤销/重做时一起回退或恢复。
- 自动补叉在自由模式和纠错模式下都会生效。
...
- [ ] 纠错模式下再次点击已填充或已标记的当前目标格子不会取消
- [ ] 自动纠正后的红色格子会永久保留
- [ ] 完成某一行后，该行剩余空白格会自动补叉
- [ ] 完成某一列后，该列剩余空白格会自动补叉
- [ ] 行列同时完成时，自动补叉不会重复写入同一格
- [ ] 自动补叉与玩家本次操作应合并为单条撤销/重做历史
...
- [ ] 难度筛选正常工作
- [ ] 胜利弹窗正确显示
- [ ] `Space` 和 `F` 都能切换填充/标记模式，且空格不会触发页面滚动
- [ ] 响应式布局在移动端正常
